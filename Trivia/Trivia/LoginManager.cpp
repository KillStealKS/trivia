...
*/
void LoggedUser::setUsername(const std::string username) { m_username = username; }
...
*/
LoginManager::LoginManager(IDatabase* database) : m_database(database), m_databaseMutex(new std::mutex), m_usersMutex(new std::mutex) {}

/*
* @brief adds a user to the database
*/
void LoginManager::signup(std::string username, std::string password, std::string email) {
	if (!m_database->doesUserExist(username))	//don't want 2 users with the same username
...
void LoginManager::login(std::string username, std::string password) {
	...
	for (auto i : m_loggedUsers) {
		if (i.getUsername() == username)
			throw std::exception(__FUNCTION__ " - user already logged in.");
	}
