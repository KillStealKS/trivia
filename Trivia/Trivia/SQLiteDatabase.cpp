...
bool SQLiteDatabase::open() {
	...
	if (doesFileExist != 0) {
		...
		std::string sqlStatement = "CREATE TABLE Users ("
			...
			"Email TEXT NOT NULL);";
		res = sqlite3_exec(m_Database, sqlStatement.c_str(), nullptr, nullptr, &errMsg);
		if (res != SQLITE_OK) {
...
bool SQLiteDatabase::doesUserExist(std::string username) {
	...
	int res = sqlite3_exec(m_Database, sqlStatement.c_str(), SQLiteDatabase::userCallback, &userList, &errMsg);
	if (res == SQLITE_OK)
...
void SQLiteDatabase::addNewUser(std::string username, std::string password, std::string email) {
	...
	int res = sqlite3_exec(m_Database, sqlStatement.c_str(), nullptr, nullptr, &errMsg);
	if (res != SQLITE_OK)
