...
*/
LoginRequestHandler::LoginRequestHandler(RequestHandlerFactory* factory, LoginManager* manager)
...
RequestResult LoginRequestHandler::handleRequest(RequestInfo reqInf) {
    ...
    if (isRequestRelevant(reqInf)) {
        ...
        if (reqInf.buffer.front() == RQ_LOGIN) {    //login
            LoginRequest userMsg = JsonResponsePacketDeserializer::deserializeLoginRequest(reqInf.buffer);
            try {
                ...
                reqRes.response = JsonResponsePacketSerializer::serializeResponse(errRes);
                reqRes.newHandler = m_handlerFactory.createLoginRequestHandler();
            }
        } else if (reqInf.buffer.front() == RQ_SIGNUP) {    //signup
            SignupRequest userMsg = JsonResponsePacketDeserializer::deserializeSignupRequest(reqInf.buffer);
            try {
                m_loginManager.signup(userMsg.username, userMsg.password, userMsg.email);
